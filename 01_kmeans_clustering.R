...
plot_customer_heatmap <- function(interactive = TRUE) {
    ...
    pct_sales_by_customer_tbl <- bike_orderlines_tbl %>% 
        ...
        mutate(label_text = str_glue("Customer: {bikeshop_name}
                                     ...
                                     Percet of Sales: {scales::percent(pct, accuracy = .1)}"))
...
plot_customer_segments <- function(k = 4, seed = 123, interactive = TRUE) {
    ...
    if(interactive) {
        ...
        g +
            geom_label_repel(aes(label = label_text), size = 2)
...
plot_customer_behavior_by_cluster <- function(top_n_products = 10, 
    ...
    top_n_tbl <-  bike_orderlines_tbl %>% 
        ...
        # remember the .by_group in arrange()!
        arrange(desc(total_qty), .by_group = TRUE) %>% 
        plotly::slice(1:top_n_products) %>% 
        ungroup() %>% 
    ...
    g <- top_n_tbl %>% 
        ...
        labs(title = str_glue("Top {top_n_products} Bike Models by Customer and Cluster"),
             x = "Category 1",
...
dump(function_names, file = "../Documents/Programming and Data Science/R/DS4B_101_R_Business_Analysis/00_scripts/plot_customer_segmentation.R")
